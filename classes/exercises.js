...
Now you get to put all of your Object Oriented programming skills into effect. 
You’re going to be writing code to model an ecommerce shopping basket.
...
class Basket {
  ...
  add (product, quantity) {
    ...
    // checks if the product id ma
   if (this.myBasket.map(item => item.id).includes(product.id)) {
     for(let objs of this.myBasket.map(item => item)){
       if (objs.id === product.id) {
  ...
  remove (productX, quantity){
    // remove quantity of prodx from basket
  }
  
  getTotal () {
    return this.myBasket.map(item => item.price).reduce((a, b) => a + b, 0)
  }

  getOrderLine(product) {
    const orderline = this.myBasket.map(item => item).filter(item => item.id === product.id)
...
const tv = new Product('PROD1', 'television', 99.99)
const iphone = new Product('PROD2', 'iphone', 999.99)
const deskchair = new Product('PROD3', 'desk chair', 67.99)

basket.add(tv, 2)
basket.add(iphone, 4)
basket.add(tv, 1)
basket.add(iphone, 1)
basket.add(deskchair, 12)

...
const withTax = (prices) => prices.map(price => {
    return price > 100 ? Number((price + (price * 0.2)).toFixed(2)) : price
})
...
const totalPerRegion = (rainfalls) => {
    return rainfalls.map(innerArray => innerArray.reduce((acc, initVal) => acc + initVal, 0))
  }